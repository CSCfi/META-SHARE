...
DJANGO_BASE = os.environ.get('DJANGO_BASE', '')
...
# Allowed hosts: Host and domain names that this django site can serve:
ALLOWED_HOSTS = ['*']

# Path to the local storage layer path used for persistent object storage.
STORAGE_PATH = '/v/MetasharePersistentStorage'
...
# Please set the SECRET_KEY to a unique random string
# The installation manual can give more information on how to create it
SECRET_KEY = os.environ.get('SECRET_KEY', 'secret_META')

LOG_FILENAME = join(tempfile.gettempdir(), os.environ.get('LOG_FILENAME', 'metashare.log'))
...
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.postgresql_psycopg2', # Add 'postgresql_psycopg2', 'mysql',
                                         # 'sqlite3', 'oracle'.
        'NAME': os.environ.get('DB_NAME'),                      # database name -- or path to file if
                                         # using sqlite3, e.g.
        ...
        'HOST': os.environ.get('DB_HOST'),                      # Set to 'localhost' for localhost.
...
SOLR_URL = 'http://10.131.9.122:8983/solr/main'
# the URL of the Solr server (or server core) which is used as a search backend
# when running tests
TESTING_SOLR_URL = 'http://10.131.9.122:8983/solr/testing'
...
#SELENIUM_DRIVER_OPTS = dict(firefox_profile="{0}/../firefox_profile".format(ROOT_PATH))
...
# You can add custom web analytics application settings here through the Django
